...
class Pet {
  lista() {
    const sql = `SELECT Pets.id, Pets.nome, Pets.tipo, Pets.observacoes, 
                ...
                FROM Pets INNER JOIN Clientes WHERE Pets.donoId = Clientes.id`;

    return executaQuery(sql).then(pets => pets.map(pet => ({
      id: pet.id, nome: pet.nome, tipo: pet.tipo, observacoes: pet.observacoes,
      dono: {
        id: pet.donoId,
        nome: pet.donoNome,
        cpf: pet.donoCpf,
  ...
  buscaPorId(res, id) {
    ...
    return executaQuery(sql).then(pets => ({
        id: pets[0].id, nome: pets[0].nome,
        tipo: pets[0].tipo, observacoes: pets[0].observacoes,
        dono: {
            id: pets[0].donoId,
  ...
  adiciona(item) {
    ...
    return executaQuery(sql).then(resposta => ({
      id: resposta.insertId, nome, donoId, tipo, observacoes
    }));
  }
